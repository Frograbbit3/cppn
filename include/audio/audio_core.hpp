...
struct Decoder {
  ...
  bool open(const char *path) {
    if (strstr(path, ".flac")) {
      ...
      if (!drmp3_init_file(m, path, nullptr)) {
        free(m);
        return false;
      }
      handle = m;
      ...
      if (!drwav_init_file(w, path, nullptr)) {
        free(w);
        return false;
      }
      handle = w;
  ...
  uint64_t read(float *buffer, uint64_t framesToRead) {
    ...
    case DecoderType::FLAC:
      return drflac_read_pcm_frames_f32((drflac *)handle, framesToRead, buffer);
    case DecoderType::MP3:
      return drmp3_read_pcm_frames_f32((drmp3 *)handle, framesToRead, buffer);
    case DecoderType::WAV:
      return drwav_read_pcm_frames_f32((drwav *)handle, framesToRead, buffer);
    default:
      return 0;
    }
  }
  void seek(uint64_t frameIndex) {
    ...
    switch (type) {
    case DecoderType::FLAC:
      drflac_seek_to_pcm_frame((drflac *)handle, frameIndex);
      break;
    case DecoderType::MP3:
      drmp3_seek_to_pcm_frame((drmp3 *)handle, frameIndex);
      break;
    case DecoderType::WAV:
      drwav_seek_to_pcm_frame((drwav *)handle, frameIndex);
      break;
    default:
      break;
    }
  ...
  void close() {
    ...
    switch (type) {
    case DecoderType::FLAC:
      drflac_close((drflac *)handle);
      break;
    case DecoderType::MP3:
      drmp3_uninit((drmp3 *)handle);
      free(handle);
      break;
    case DecoderType::WAV:
      drwav_uninit((drwav *)handle);
      free(handle);
      break;
    default:
      break;
